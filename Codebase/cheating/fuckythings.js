...
function emjack() {
	...
	window.ej={
		...
		users: users,
		settings: +localStorage.ejs || AUKICK | UNOTES | DSPFMT,
		};
	notes=ej.notes;
	afk=(ej.settings & JEEVES)===JEEVES;
	if(ej.settings & GRAPHI) {
	...
	ej.cmds={
		"~": function(data) {
			if(round.state && !ranked) {
				if(1 == 1 && !meetd.say && round.state&1===0) {
		...
		lcopy.sc,
		{
			...
			callback: function(topic) {
				if(this.topics[topic]) {
					...
					log("You can also press shift instead of enter to send commands in lobby chat");
					}
				},
			topics: {
	...
	ej.lbot=[
			...
			},
		{
			...
			response1: [
				"@$1 o-oh...", "/me blushes", "/me blushes..."
				],
			response2: [
				"@$1 y-you too...", "@$1 i like you too...", "s-same..."
				]
			},
		{
			name: "Advice",
			...
			},
		{
			...
			responses: [
				"13/f/cali", "21/neuter/sanfran", "19/ladyboy/thailand",
				"12/m/em", "40/m/dtf", "0/f/china", "18/f/barelylegalcamsdotcom",
				"?/?/master's basement", "master says not to talk to strangers on epicmafia"
				]
			},
		{
			name: "Roll dice",
			...
			},
		{
			...
			responses: [
				"who", "you suck", "who are you", "you own an account on epicmafia.com"
				]
			},
		{
			name: "Roulette",
	...
	window.addEventListener("click", function(event) {
		if(event.target.classList.contains("meet_username")) {
			ej.run("vote "+(
